...
+ (void)initialize {
     ...
     */
    // 初始化类之前收到它的第一个消息。1。运行时发送初始化消息类线程安全的方式。2。初始化每个类只被调用一次。如果你想执行初始化类和独立类别的类,你应该实现加载方法。
...
#pragma mark - 便利的构造方法
+ (void)excuteInMainQueue:(dispatch_block_t)block {
    
    NSParameterAssert(block);
    dispatch_async(mainQueue.dispatchQueue, block);
}

+ (void)excuteInGlobalQueue:(dispatch_block_t)block {
